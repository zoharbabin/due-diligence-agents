...
if echo "$COMMAND" | grep -q "git commit"; then
    ...
    # Run unit tests
    if ! python -m pytest tests/unit/ -x -q --tb=no 2>/dev/null; then
        echo "BLOCKED: Unit tests must pass before committing. Run: pytest tests/unit/ -x" >&2
        exit 2
    fi

    # Run type check
    if ! python -m mypy src/ --strict --no-error-summary 2>/dev/null; then
