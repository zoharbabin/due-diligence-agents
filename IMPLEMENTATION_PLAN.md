...
- [x] `pytest tests/ -x` passes — 2257 unit tests, 17 integration, 9 E2E (3 skipped without API key)
...
| Category | Count |
|----------|-------|
| Unit tests (models, utils, entity resolution, extraction, config, hooks, tools, orchestrator, agents, reporting, validation, search, glm_ocr, type safety, visual grounding, entity dedup, OCR registry, layout PDF, HTML report, HTML renderers, report rendering, extraction backend, turn limits, severity recalibration, executive synthesis, data quality classification, follow-up verification, assessment, red flag scanner, cost tracker, batch scheduler, financial impact) | 2257 |
