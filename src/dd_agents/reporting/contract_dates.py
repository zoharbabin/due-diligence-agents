...
classifies subjects into one of five statuses.  Runs when
``source_of_truth.subject_database`` exists in ``deal-config.json``.
"""
...
class ContractDateReconciler:
    """Classify subjects by contract status using database + data room evidence."""
    ...
    def reconcile(
        self,
        subject_database: list[dict[str, Any]],
        ...
        findings:
            ...
            subject name (not safe_name).  Used to look for renewal
            evidence such as auto-renewal clauses, POs, etc.
        subjects:
            Optional explicit subject list.  When not supplied, all entries
            in *subject_database* are processed.
        run_id:
        ...
        for db_row in subject_database:
            ...
            entry = self._classify(
                ...
                arr=arr,
                subject_findings=findings.get(subject_name, []),
                db_row=db_row,
            )
    ...
    def _classify(
        ...
        """Classify a single subject into one of the five statuses."""
        # Parse end date
        end_date = self._parse_date(end_date_str)

        # No contract documents at all?
        has_contracts = bool(subject_findings)
        ...
        has_auto_renewal = self._has_auto_renewal_evidence(subject_findings)
        has_renewal_evidence = self._has_renewal_evidence(subject_findings)
        ...
        if has_auto_renewal:
            return ContractDateReconciliationEntry(
                ...
                evidence_file=self._get_evidence_file(subject_findings),
            )
