...
PHASE 3: INVENTORY (Steps 6-12)
  Step  6: Build inventory -> customers.csv, counts.json
  ...
  Step  9: Customer-mention index -> customer_mentions.json
  Step 10: Inventory integrity verification (total files = customer + reference, no orphans)
  Step 11: [IF source_of_truth.customer_database] Contract date reconciliation
  Step 12: [IF incremental mode] Customer classification -> classification.json
...
PHASE 4: AGENT EXECUTION (Steps 13-17)
  ...
  Step 17: Validate customer coverage
           *** BLOCKING GATE: every customer must have output from all 4 agents ***
           - Count {customer_safe_name}.json files per agent directory
           - Re-spawn for missing customers (one retry)
           - Verify no aggregate files (_global.json, batch_summary.json)
           - Verify coverage_manifest.json exists per agent
           - Spot-check clean-result entries for customers with zero findings
...
[Steps 6-10: Inventory Construction]
    |
    +--> _dd/forensic-dd/inventory/customers.csv
    +--> _dd/forensic-dd/inventory/counts.json
    +--> _dd/forensic-dd/inventory/reference_files.json
    +--> _dd/forensic-dd/inventory/customer_mentions.json
...
[Steps 14-15: Prompt Builder + Route References]
    ...
    |    Each contains: deal context, full customer list with safe_names,
...
[Steps 16-17: 4 Specialist Agents in Parallel]
    |
    |  Legal Agent --->  {RUN_DIR}/findings/legal/{customer_safe_name}.json
    |                    {RUN_DIR}/findings/legal/gaps/{customer_safe_name}.json
    |                    {RUN_DIR}/findings/legal/coverage_manifest.json
    |                    {RUN_DIR}/audit/legal/audit_log.jsonl
    |
    |  Finance Agent --> {RUN_DIR}/findings/finance/{customer_safe_name}.json
    |                    {RUN_DIR}/findings/finance/gaps/{customer_safe_name}.json
    |                    {RUN_DIR}/findings/finance/coverage_manifest.json
    |                    {RUN_DIR}/audit/finance/audit_log.jsonl
    |
    |  Commercial ----> {RUN_DIR}/findings/commercial/{customer_safe_name}.json
    |                   {RUN_DIR}/findings/commercial/gaps/{customer_safe_name}.json
    |                   {RUN_DIR}/findings/commercial/coverage_manifest.json
    |                   {RUN_DIR}/audit/commercial/audit_log.jsonl
    |
    |  ProductTech ---> {RUN_DIR}/findings/producttech/{customer_safe_name}.json
    |                   {RUN_DIR}/findings/producttech/gaps/{customer_safe_name}.json
...
[Steps 19-22: Judge Agent (optional)]
    ...
    |
    v
[Steps 23-31: Reporting Lead + Validation Gates]
    |
    +--> {RUN_DIR}/findings/merged/{customer_safe_name}.json
    +--> {RUN_DIR}/findings/merged/gaps/{customer_safe_name}.json
...
| File | Purpose |
|------|---------|
| `findings/legal/*.json` | Legal agent per-customer output |
| `findings/finance/*.json` | Finance agent per-customer output |
| `findings/commercial/*.json` | Commercial agent per-customer output |
| `findings/producttech/*.json` | ProductTech agent per-customer output |
| `findings/{agent}/gaps/*.json` | Per-agent per-customer gap files |
...
| `contract_date_reconciliation.json` | Date reconciliation (if customer_database exists) |
...
| `_dd/forensic-dd/inventory/customers.csv` | Customer registry (group, name, path, file count) |
| `_dd/forensic-dd/inventory/counts.json` | Aggregate counts (files, customers, by extension, by group) |
| `_dd/forensic-dd/inventory/reference_files.json` | Global reference file catalog with routing |
| `_dd/forensic-dd/inventory/customer_mentions.json` | Customer names found in reference files |
...
### Reporting Lead (sequential, after Judge or specialists)
...
| `enforce_output_paths` | `Write` | Specialists can only write to `{RUN_DIR}/findings/{agent}/`; Reporting Lead to `{RUN_DIR}/findings/merged/` and `{RUN_DIR}/report/` |
...
|------|-----------|-----------------|
| `verify_coverage_before_stop` | Specialists | Blocks stop if `count({customer_safe_name}.json) < expected_customers` |
...
async def verify_coverage_before_stop(input_data, tool_use_id, context):
    """Prevent specialist from stopping before processing all customers."""
    ...
    if actual < expected_customers:
        return {
            "decision": "block",
            "reason": (
                f"Produced {actual}/{expected_customers} customer JSONs. "
...
| Hook | Matcher | What it enforces |
|------|---------|-----------------|
| `track_output_count` | `Write` | Increments per-agent customer output counter |
| `validate_customer_json` | `Write` | Validates written customer JSON against CustomerJSON Pydantic model |
...
Agent attempts: Write to findings/legal/acme_corp.json
  ...
  3. PostToolUse fires:
     ...
     - validate_customer_json: parse file, check required fields -> feedback if invalid

Agent attempts: Stop (finish execution)
  4. Stop hook fires:
     - verify_coverage_before_stop: count = 28, expected = 34 -> BLOCK
     - Agent receives: "Produced 28/34 customer JSONs. Continue processing."
  5. Agent continues processing remaining 6 customers
